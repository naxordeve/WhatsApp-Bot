...
const serialize = async (conn, msg) => {
    ...
    if (msg.message) {
        ...
        msg.groupClose = async () => {
            if (!msg.isGroup) throw new Error('Not a group');
            if (!msg.isAdmin && !msg.fromMe) throw new Error('Not an admin');
        ...
        msg.groupOpen = async () => {
            ...
            return 'Group opened successfully';
        };
        ...
        msg.add = async (number) => {
            ...
            try {
                ...
                if (res[0].status === "409") {
                    ...
                    return `Cannot add directly. Invite link: https://chat.whatsapp.com/${code}`;
                }
                return `@${user.split('@')[0]} added to group`;
            } catch (error) {
                const code = await conn.groupInviteCode(msg.user);
                return `Failed to add. Invite link: https://chat.whatsapp.com/${code}`;
            }
        };
