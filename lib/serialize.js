...
const serialize = async (conn, msg) => {
    ...
    if (msg.message) {
        ...
        msg.download = async () => {
            ...
            if (!msg.fromMe) return msg.reply("You can only edit your own messages.");

            try {
                const editedMsg = await conn.sendMessage(msg.user, {
                    text: content,
                    edit: msg.key
                });
                return editedMsg;
                ...
                return msg.reply("Failed to edit the message.");
            }
        };
